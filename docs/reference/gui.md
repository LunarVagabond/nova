...
- **`src/components/`** — `Sidebar.vue`/`ProjectPanel.vue`/`CollectionNode.vue`
  ...
  below) for the response pane's Diff tab; `ChangesPanel.vue` for the
  containing git repository's working-tree changes (see below), reachable
  from the top bar's git-branch-icon action; `Modal.vue` is the shared
