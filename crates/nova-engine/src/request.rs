...
impl RequestFile {
    ...
    /// Write an edited [`WebSocketDraft`] — URL/headers/messages — back to
    /// this file on disk as a WebSocket connection declaration, going
    /// through [`ParsedWebSocketRequest::to_nova_string`] rather than
    /// nova-app hand-rolling `.nova` syntax — the WebSocket counterpart to
    /// [`RequestFile::write`].
...
/// [`ParsedRequest`]/[`ParsedWebSocketRequest`]/[`ParsedSseRequest`] — the
/// same lightweight, best-effort spirit as the `method` peek collection
/// discovery has always done. Defaults to `"http"` when the file declares
/// no `protocol:` line at all (the vast majority of files, since HTTP is
...
/// [`RequestDraft::body_text`] carries for a request's body regardless of
/// its type — into its individual [`MultipartField`]s, using the boundary
/// declared in `headers`' `Content-Type`.
///
/// A thin, standalone wrapper around the same dispatch
/// [`RequestBody::from_text`] uses internally, so a caller that only wants
/// structured multipart fields (the GUI's multipart body editor, so it
/// never has to parse `.nova` body text itself) doesn't need to match on a
/// whole [`RequestBody`]. `body_text` that's empty comes back as no
/// fields at all, matching [`RequestBody::None`]; anything else that isn't
/// actually multipart-shaped (a mismatched `Content-Type`) is an error.
...
impl ParsedWebSocketRequest {
    ...
    /// panel — the WebSocket counterpart to [`ParsedRequest::to_draft`].
    ...
    /// [`RequestFile::parse_websocket`]/[`parse_nova_websocket`], and the
    /// WebSocket counterpart to [`ParsedRequest::to_nova_string`]. Always
    /// succeeds: unlike an HTTP request's body, a message list has no
    /// content-type-dependent serialization that could fail.
...
/// once connected — the WebSocket counterpart to [`RequestDraft`]. Every
/// field here round-trips through [`RequestFile::write_websocket`]
