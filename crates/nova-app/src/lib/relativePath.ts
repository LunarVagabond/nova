...
// the multipart and binary body editors' file references — a
// `MultipartField.file_path` (or a `RequestBody::Binary` path) is always
// stored relative to the project root (see `nova_engine::execute`), never
// as an absolute path baked into the `.nova` file.
//
 ...
 *
 * A `MultipartField.file_path`/binary body path is only ever meant to be a
 * project-relative reference (the engine rejects anything else at send
 * time — see `nova_engine::execute::resolve_project_file_path`), so a
 * picked file outside the project has nothing sensible to write here;
 * returning `null` lets the caller refuse the attachment instead of
 * silently saving a path that won't resolve for anyone else who opens the
 * project, or resolves to something else entirely.
 */
